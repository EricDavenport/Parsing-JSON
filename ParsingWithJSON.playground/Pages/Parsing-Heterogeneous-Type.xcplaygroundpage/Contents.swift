...
// Parsing Hete4rogeneous (mixed tyoes) Type
//    ex. postcode on occasion is an Int or string
//    this will to decoding erros while parsing
...
enum Postcode: Decodable {
  // handle the two cases on the JSON 'postcode" property where
  //      postcode can either be am (Int) or a (String)
  
  // we will also use associative values to capture value of postcode
  // e.g if it's an Ibnt wre wikl capture the Int value
  case int(Int)
  case string(String)
  ...
  init(from decoder: Decoder) throws {
    ...
    if let intValue = try? decoder.singleValueContainer().decode(Int.self)  {   //decoder.singleValueContainer().decode() - allows you to decode native types(string, bool, int, etc,)
      self = .int(intValue)
      return
    }
